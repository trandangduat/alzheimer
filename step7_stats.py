import os
import subprocess
from config import SUBJECTS_DIR, SCHAEFER_GCS_ROOT, FREESURFER_AVERAGE_DIR, run_cmd_logged
...
def run_cmd(cmd):
    ...
    try:
        ...
        print(f"[ERROR] Error executing command: {cmd}")
...
def run_step7_full_stats(sid):
    ...
    """
    print(f"\n[7/7] [PROCESSING] Generating Schaefer Atlas Stats for {sid}...")
    ...
    try:
        ...
        run_cmd(f"mris_sphere -threads {threads} -seed 1234 {os.path.join(surf_dir, 'lh.inflated')} {os.path.join(surf_dir, 'lh.sphere')}")
        run_cmd(f"mris_sphere -threads {threads} -seed 1234 {os.path.join(surf_dir, 'rh.inflated')} {os.path.join(surf_dir, 'rh.sphere')}")

        # 5. mris_register (sphere.reg)
        run_cmd(f"mris_register -curv -threads {threads} {os.path.join(surf_dir, 'lh.sphere')} {os.path.join(FREESURFER_AVERAGE_DIR, 'lh.folding.atlas.acfb40.noaparc.i12.2016-08-02.tif')} {os.path.join(surf_dir, 'lh.sphere.reg')}")
        run_cmd(f"mris_register -curv -threads {threads} {os.path.join(surf_dir, 'rh.sphere')} {os.path.join(FREESURFER_AVERAGE_DIR, 'rh.folding.atlas.acfb40.noaparc.i12.2016-08-02.tif')} {os.path.join(surf_dir, 'rh.sphere.reg')}")
        ...
        print("[SUCCESS] Schaefer Atlas Stats generated successfully.")
        return stats_dir
        
    except Exception as e:
        # print error but don't re-raise, let pipeline continue or handle it? 
        # For now, print error.
        print(f"[ERROR] Failed to generate Schaefer Atlas Stats: {e}")
        return None
