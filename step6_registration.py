...
def run_step6_registration(subject_id):
    ...
    for hemi in SUGAR_HEMIS:
        ...
        if (
            not os.path.exists(sphere)
            or not os.path.exists(curv)
            or not os.path.exists(sulc)
        ):
            print(f" >>> Generating missing FreeSurfer files for {hemi}...")

            smoothwm = os.path.join(surf_dir, f"{hemi}.smoothwm")
            print(f"     -> Running mris_smooth (this may take a few minutes)...")
            run_cmd_logged(
                ["mris_smooth", "-n", "3", "-nw", "-seed", "1234", white, smoothwm]
            )

            inflated = os.path.join(surf_dir, f"{hemi}.inflated")
            print(f"     -> Running mris_inflate (this may take a few minutes)...")
            run_cmd_logged(["mris_inflate", smoothwm, inflated])

            print(f"     -> Running mris_place_surface to create curv surface...")
            run_cmd_logged(["mris_place_surface", "--curv-map", white, "2", "10", curv])

            print(f"     -> Running mris_sphere (this may take a few minutes)...")
            run_cmd_logged(
                ["mris_sphere", "-threads", "8", "-seed", "1234", inflated, sphere]
            )

        if not all(os.path.exists(p) for p in [sphere, curv, sulc]):
            print(
                f"[ERROR] Could not generate missing inputs for {hemi} registration. Skipping."
            )
            continue
        ...
        try:
            ...
            sugar_result = os.path.join(
                sugar_out_dir, subject_id, "surf", f"{hemi}.sphere.reg"
            )
